...
from models.explorer import (get_sorted_files, get_file_info, format_file_size, datetimeformat, query_string, )
...
class Admins(db.Model):
    ...
    def password(self, password):
        """
        Sets the password to be used for authenticating. This is a string of length 256 and should be used in
        conjunction with the : meth : ` password_hash ` method
        """
    ...
    def verify_password(self, password):
        ...
        Verify a password against the hash. This is used to verify the user's password before logging in to the system
         ...
         Returns: True if the password matches the hash False otherwise. Note that a return value of False does not
         mean that the password was not correct
        """
...
def login_required(route):
    ...
    This decorator checks if a username is present in the session data. If not, it flashes an error message
    indicating the user needs to log in and redirects them to the login page. If a username is found, it retrieves
    the corresponding Admin user object from the database using SQLAlchemy. If the user is not found, it also flashes
    an error message and redirects to the login page. Otherwise, the original route function is called,
    ...
    def wrapper(*args, **kwargs):
        ...
        This function checks the session for a username and retrieves the corresponding Admin user if found. If not
        logged in or user not found, it displays an error message and redirects to log in. Otherwise, it calls the
        original route function with the provided arguments.
...
def upload():
    ...
    # Create the base directory if it doesn't t exist.
...
def directories():
    ...
    return render_template('directories.html',
                           files=files_on_page, current_directory=current_directory, page=page,
                           total_pages=total_pages, title="Directories")
...
def view_file(filepath):
    ...
    return render_template("view_file.html",
                           title="View File", file_path=file_path.resolve(), file_info=file_info, )
...
def retrieve_selected_path():
    ...
    if request.method == "POST":
        ...
        if os.path.isdir(selected_file):
            ...
            return render_template("query_db.html", folder=selected_file, title="Query DB")

        elif os.path.isfile(selected_file):
            try:
                if selected_file.endswith('.pdf'):
                    return render_template("split_enc.html",
                                           selected_file=selected_file, title="Split Encrypt")
...
def split_encrypt():
    ...
    try:
        ...
        thread = Thread(target=splitter, args=(str(file), file_path, task_id))
        thread.start()
...
def results():
    ...
    return render_template("results.html",
                           active=len(active_found), inactive=len(inactive), unknown=len(unknown),
                           folder=folder, )
...
def view_users(category):
    ...
    return render_template("view_users.html", category=category, users=users.items, pagination=users,
...
def back_to_result():
    ...
    return render_template("results.html",
                           active=active, inactive=inactive, unknown=unknown, folder=folder)
...
def send_mail():
    ...
    folder = request.form.get("folder")
    task_id = str(time.time())  # Generate a unique task ID
    progress_data[task_id] = {"logs": [], "errors": [], "status": "running", "total": 0, "sent": 0, "failed": 0,
    ...
    return render_template("results_visual.html",
                           task_id=task_id, folder=folder_encoded, filename=file_name, )
    ...
    with app.app_context():
        ...
        for file in files:
            ...
            log_entry = {"timestamp": timestamp,
                         "message": (error_message if mail_att else f"Email notification failed to {email}."),
                         "file": file,
                         "email": email, }
            ...
            if mail_att:
                ...
                progress_data[task_id]["errors"].append(
                    {"file": file, "email": email, "error": error_message, "timestamp": timestamp, })
...
def retry_page():
    ...
    return render_template("retry_page.html", task_id=task_id, folder=folder, filename=filename)
...
def retry_logs():
    ...
    Args:
        The unique identifier for the email sending task.
...
def retry_send_mail():
    ...
    return render_template("results_visual.html",
                           task_id=task_id, folder=quote(folder), filename=file_name)
...
def export_logs():
    ...
    logs_csv = generate_csv(
        [{"timestamp": log["timestamp"], "message": log["message"], "file": log["file"], "email": log["email"], } for
         log in progress_data[task_id]["logs"]], ["timestamp", "message", "file", "email"], )
    errors_csv = generate_csv(
        [{"timestamp": error["timestamp"], "file": error["file"], "email": error["email"], "error": error["error"], }
         for error in progress_data[task_id]["errors"]], ["timestamp", "file", "email", "error"], )
...
def add_user():
    ...
    try:
        ...
        new_user = User(email=email, ippis=ippis, first_name=first_name, surname=surname, phone=phone, active=active, )
...
def manage_users():
    ...
    return render_template('manage_users.html', users=users.items, pagination=users, total_users=total_users)
...
def manage_admins():
    ...
    return render_template('manage_admins.html', admins=admins.items, pagination=admins, total_admins=total_admins)
